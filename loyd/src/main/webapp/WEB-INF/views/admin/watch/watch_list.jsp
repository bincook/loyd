		...
		<form name="del" method="post" action="delete">
			...
			<table width="1000" align="center">
				...
				<tr style="display: none" id="ctn_show">
					<td><i class="bi bi-trash" onclick="deletevalue()"></i> <!--     삭제 icon누르면 삭제 -->
						<i class="bi bi-eye-slash"></i></td>
				</tr>
				<tr>
					</td>
					<!--  화면에 표시되는 전체 체크박스 -->
					<td onclick="view()"><input id="allcheck" type="checkbox"
						name="allcheck"></td>
					...
					<td><i class="bi bi-tags"></i> / <i class="bi bi-smartwatch"></i>
					</td>
				...
				<c:forEach items="${watch_list}" var="wdto">
					<tr>
						<!--  하나의 상품에 대한 체크박스 -->
						<td onclick="view()"><input name="rowcheck" type="checkbox"
							value="${wdto.id}"></td>
						<td>${wdto.id }</td>
		   ...
		   -->
						...
						<td>${wdto.brand} <br> <a href="content?id=${wdto.id}"
							style="color: #22741C;">${wdto.name}</a>
						</td>
						<td><fmt:formatNumber value="${wdto.price}" pattern="#,###" />
						</td>
						<td>${wdto.category}</td>
						<td>${wdto.discount}</td>
						<td>${wdto.kind}</td>
