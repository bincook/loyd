...
<div class="container">
	<h1 style="text-align:center;">WOMEN WATCH LIST</h1>
	<!-- 1줄 (12칸) 차지-->
...
<table align="center">
	<tr>
      ...
      <c:if test="${pstart != 1}">
       <a href="women_product_list?page=${pstart-1}"> << </a>
      </c:if>
      <c:if test="${pstart == 1}"> 
       <<
      </c:if>
       <!-- 현재페이지 이전 1페이지 이동 -->
      <c:if test="${page != 1}"> 
       <a href="women_product_list?page=${page-1}"> < </a>
      </c:if>
      <c:if test="${page == 1}"> 
       <
      </c:if>
       <!-- 현재페이지 기준으로 이동할 수 있는 페이지  -->
        <c:forEach begin="${pstart}" end="${pend}" var="i">
          <!-- 현재페이지 색은 다르게 => 빨강 -->
         <c:if test="${page != i}"> 
          <a href="women_product_list?page=${i}"> ${i} </a>
         </c:if>
         <c:if test="${page == i}"> 
          <a href="women_product_list?page=${i}" style="color:red"> ${i} </a>
         </c:if>
        </c:forEach>
      ...
      <c:if test="${page_cnt != pend}"> 
       <a href="women_product_list?page=${pend+1}"> >> </a>
      </c:if>
      <c:if test="${page_cnt == pend}"> 
       >>
      </c:if> 
